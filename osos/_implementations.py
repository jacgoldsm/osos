...
def mode_func(series: pd.Series, *args, **kwargs):
    ...
    try:
        return (
            roll.apply(pd.Series.mode).reset_index()[series.name].astype(series.dtype)
        )
    except pd.errors.IntCastingNaNError:
...
def kurtosis_func(series: pd.Series, *args, **kwargs):
    ...
    try:
        return (
            roll.apply(pd.Series.kurtosis)
            .reset_index()[series.name]
            .astype(series.dtype)
        )
    except pd.errors.IntCastingNaNError:
...
def lag_func(series: pd.Series, *args, **kwargs):
    ...
    try:
        return (
            roll.shift(periods=offset, fill_value=default)
            .sort_index()
            .reset_index()[series.name]
            .astype(series.dtype)
        )
...
def percentile_approx_func(
    ...
    accuracy = (
        pd.Series(accuracy) if isinstance(accuracy, pd.Series) else pd.Series(accuracy)
    )
...
@overload
def struct_func(__cols: Union[list[pd.Series], tuple[pd.Series, ...]]):
    ...
...
def log_func(arg1: pd.Series | float, arg2: Optional[pd.Series] = None):
...
def nth_value_func(col: pd.Series, offset: int, ignoreNulls: Optional[bool] = False):
...
def make_date_func(year: pd.Series, month: pd.Series, day: pd.Series):
...
def add_months_func(start: pd.Series, months: Union[pd.Series, int]):

    months = pd.Series(months) if isinstance(months, int) else months
    raise NotImplementedError


def months_between_func(date1: pd.Series, date2: pd.Series, roundOff: bool = True):

    raise NotImplemented("months_between", pd.Series(date1), pd.Series(date2), roundOff)
...
def from_unixtime_func(timestamp: pd.Series, format: str = "yyyy-MM-dd HH:mm:ss"):
...
def assert_true_func(col: pd.Series, errMsg: Optional[Union[pd.Series, str]] = None):
