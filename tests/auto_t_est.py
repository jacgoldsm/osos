...
# osos.functions.hex:
OsosSession.createDataFrame([("ABC", 3)], ["a", "b"]).select(
    hex("a"), hex("b")
).collect()
...
OsosSession.createDataFrame([(21,)], ["a"]).select(
    shiftleft("a", 1).alias("r")
).collect()

# osos.functions.shiftright:
OsosSession.createDataFrame([(42,)], ["a"]).select(
...
df = OsosSession.createDataFrame(
    [("1997-02-28 10:30:00", "1996-10-30")], ["date1", "date2"]
)
...
df = OsosSession.createDataFrame(
    [(1, [1, 2, 3, 4]), (2, [3, -1, 0])], ("key", "values")
)
df.select(exists("values", lambda x: x < 0).alias("any_negative")).show()

# osos.functions.forall:
df = OsosSession.createDataFrame(
    [(1, ["bar"]), (2, ["foo", "bar"]), (3, ["foobar", "foo"])], ("key", "values")
)
df.select(forall("values", lambda x: x.rlike("foo")).alias("all_foo")).show()

# osos.functions.filter:
df = OsosSession.createDataFrame(
    [(1, ["2018-09-20", "2019-02-03", "2019-07-01", "2020-06-01"])], ("key", "values")
)
...
df.select(filter("values", after_second_quarter).alias("after_second_quarter")).show(
    truncate=False
)
...
df.select(zip_with("xs", "ys", lambda x, y: x**y).alias("powers")).show(
    truncate=False
)
...
row = df.select(
    transform_keys("data", lambda k, _: upper(k)).alias("data_upper")
).head()
sorted(row["data_upper"].items())

# osos.functions.transform_values:
df = OsosSession.createDataFrame(
    [(1, {"IT": 10.0, "SALES": 2.0, "OPS": 24.0})], ("id", "data")
)
row = df.select(
    transform_values(
...
df = OsosSession.createDataFrame(
    [(1, {"foo": 42.0, "bar": 1.0, "baz": 32.0})], ("id", "data")
)
...
row = df.select(
    map_zip_with("base", "ratio", lambda k, v1, v2: round(v1 * v2, 2)).alias(
        "updated_data"
    )
).head()
...
df1 = OsosSession.createDataFrame(
    [(1, "Bob"), (0, None), (2, "Alice")], ["age", "name"]
)
df1.sort(asc_nulls_first(df1.name)).show()

# osos.functions.asc_nulls_last:
df1 = OsosSession.createDataFrame(
    [(0, None), (1, "Bob"), (2, "Alice")], ["age", "name"]
)
...
OsosSession.createDataFrame([("cat",), ("ð",)], ["cat"]).select(
    bit_length("cat")
).collect()
...
OsosSession.createDataFrame([(5,)], ["a"]).select(
    format_number("a", 4).alias("v")
).collect()
...
OsosSession.createDataFrame([("ab cd",)], ["a"]).select(
    initcap("a").alias("v")
).collect()
...
OsosSession.createDataFrame([("ABC ",)], ["a"]).select(
    length("a").alias("length")
).collect()
...
df0 = OsosSession.createDataFrame(
    [
        (
            "kitten",
            "sitting",
...
df.select(
    regexp_replace("str", col("pattern"), col("replacement")).alias("d")
).collect()

# osos.functions.unbase64:
df = OsosSession.createDataFrame(
    ["U3Bhcms=", "UHlTcGFyaw==", "UGFuZGFzIEFQSQ=="], "STRING"
)
...
OsosSession.createDataFrame([("translate",)], ["a"]).select(
    translate("a", "rnlt", "123").alias("r")
).collect()
